// functions/api/upload-video.js
// Uploads a video file to Cloudflare R2 generated-videos bucket
// Uses same AWS Signature V4 as upload-image.js
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const url = `https://${env.R2_ACCOUNT_ID}.r2.cloudflarestorage.com/${env.R2_VIDEO_BUCKET_NAME}/${key}`;
    ...
    if (!uploadResponse.ok) {
      ...
      throw new Error(`R2 video upload failed: ${uploadResponse.status} - ${text}`);
    }

    const publicUrl = `${env.R2_VIDEO_PUBLIC_URL}/${key}`;
...
async function signR2Request({ method, url, body, contentType, accessKeyId, secretAccessKey, region, service }) {
